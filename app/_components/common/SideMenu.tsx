...
export default function SideMenu({ isOpen, handleMenuDown }: SideMenuProps) {
  ...
  return (
    <div
      className={`${
        isOpen ? "w-screen" : "w-0"
      } fixed left-0 top-0 z-30 mt-28 h-screen overflow-x-hidden transition-all duration-300 sm:hidden`}
    >
      <div
      ...
      >
        <div className="flex h-[calc(100vh-115px)] flex-col gap-8 overflow-y-scroll bg-slate-50 p-10 sm:gap-4">
          <ul>
            <li
